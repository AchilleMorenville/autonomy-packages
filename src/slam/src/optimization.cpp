  ...
  for (int i = 0; i < (int) source_edge_points->points.size(); i++) {
    ...
    if (point_search_sq_dist[4] < 0.3) { // The last point is close enough
      ...
      if (mat_E_cov.at<float>(0, 0) > 3 * mat_E_cov.at<float>(0, 1)) { // We are on an edge
        ...
        /*
        float s = 1 - 0.9 * fabs(dist);

        coeff.x = s * coeff_x;
        coeff.y = s * coeff_y;
        coeff.z = s * coeff_z;
        coeff.intensity = s * dist;

        if (s > 0.1) {
          coeffs_corr->push_back(coeff);
          weights_corr->push_back(1);
  ...
  for (int i = 0; i < (int) source_flat_points->points.size(); i++) {
    ...
    if (point_search_sq_dist[4] < 0.3) {
      ...
      if (plane_valid) {
        ...
        /*
        float s = 1 - 0.9 * fabs(dist) / std::sqrt(std::sqrt(point_transformed.x * point_transformed.x +
          point_transformed.y * point_transformed.y + point_transformed.z * point_transformed.z));
