...
class SpotConnectClient(Node):
	...
	def __init__(self):
		...
		self.connect_fiducials = False
		self.connect_odometry = True
		...
		while self.connect_fiducials and not self.client_spot_driver_fiducial_connect.wait_for_service(timeout_sec=1.0):
			self.get_logger().info('fiducial service not available')
		while self.connect_odometry and not self.client_spot_driver_odometry_connect.wait_for_service(timeout_sec=1.0):
			self.get_logger().info('odometry service not available')

	def send_request_spot_driver_connect(self):
		
		if self.connect_odometry:
			req = Trigger.Request()
